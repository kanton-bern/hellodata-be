...
- **Synchroniser**: The Synchroniser reads the data from the Datamart and converts it into a structured format (CSV, XML, JSON). In a further step, this is stored on a filestorage.  
...
4. **Transform data into a file format**: The data is transformed into one or more file formats (e.g. JSON, XML). 
