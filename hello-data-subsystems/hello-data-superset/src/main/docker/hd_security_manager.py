...
#
# HelloDATA custom Superset security manager: OAuth to Keycloak with portal
# cookie-SSO (a valid portal Keycloak token in the auth.access_token cookie logs
# the user straight in). The Keycloak realm issuer comes from env
# (KEYCLOAK_BASE_URL = the full realm issuer URL), so this module ships verbatim
# in the image and works per environment. Referenced from superset_config.py:
...
class HdAuthOAuthView(AuthView):
  ...
  def oauth_authorized(self, provider: str) -> WerkzeugResponse:
    ...
    if provider not in self.appbuilder.sm.oauth_remotes:
      ...
      return redirect(self.appbuilder.sm.get_url_for_login)
    try:
